...
内容：MACD/KDJ/RSI/ATR/DMI/BOLL 等 80 余个指标函数，基于 tdx_funcs 序列原语实现

约定：
- 输入为 numpy 数组（K线字段：CLOSE/HIGH/LOW/OPEN/VOL/AMOUNT），输出为 numpy 数组或元组
- 依赖通达信未实现函数（WINNER/INDEXC/CAPITAL 等）的指标调用时会抛 NameError
- 本模块为项目唯一的指标公式来源，禁止在其他模块重复实现公式
"""
