...
class TdxSource:
    ...
    def fetch_prev_n_day_vol(self, code: str, n: int = 5, date: str = None) -> dict:
        ...
        try:
            ...
            if df is None or df.empty:
                print(f"[TdxSource] 未获取到 {code} 的日线数据")
                return None
            ...
            if date:
                ...
                if not mask.any():
                    print(f"[TdxSource] 日线数据不包含 {date} 之前的数据")
                    return None
                ...
                if loc < n:
                    print(f"[TdxSource] {code} 在 {date} 之前数据不足 {n} 日")
                    return None
            ...
            print(f"[TdxSource] 获取过去n日日线数据失败: {e}")
            return None